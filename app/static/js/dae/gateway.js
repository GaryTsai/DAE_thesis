...
function gateway_initset(response) {
    var row = $('<div class="card gateway ' + response['gateway_id'] + '">\
            ...
            <form action="/client_page" method="POST" style="display:inline" >\
...
function new_gateway(this_button) {
    ...
    // lvami project_name equal to account
    lvami_account = $('#lvami_account').text();
...
function update_gateway(this_button) {
    ...
    $.ajax({
        ...
        error: function (response) {
            swal('Gateway更新成功', response['state'], 'error')
        }
    });
}
// 空值檢查
